...
{
    template<unsigned int nDefValue>
    CLongHexNumber<nDefValue>::CLongHexNumber() {}

    template<unsigned int nDefValue>
    int CLongHexNumber<nDefValue>::FromString(std::wstring &sValue)
    {
        if ( !Parse(sValue) )
        {
    ...
    template<unsigned int nDefValue>
    std::wstring CLongHexNumber<nDefValue>::ToString() const
    {
        return XmlUtils::IntToString(this->m_eValue, L"%08X");
    }

    template<unsigned int nDefValue>
    bool CLongHexNumber<nDefValue>::Parse(std::wstring &sValue)
    {
        if ( sValue.length() < 8 )
            return false;
    ...
    template<unsigned int nDefValue>
    int	CLongHexNumber<nDefValue>::HexToInt(int nHex, bool &bResult)
    {
